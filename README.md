...
- `reaction_choice`: select the first input from the displayed choices.
- `timing`, `dialogue_timing`: press inside the success window.
...
- `memory`: memorize the sequence, then enter it after it is hidden.
- `rhythm`: press each input near its timing marker.
- `analog_precision`, `balance`: press when the moving marker reaches the center.
